\documentclass[journal,10pt,onecolumn,compsoc]{IEEEtran} 
   ...
   \iffalse
	...
	The context view is an abstract portion or container of each page’s content. 
    \subsubsection{Element} 
	The container itself will be empty until it is interacted with by the user. Depending on the context of what the user does, it will load different content within the view.
    \subsubsection{Functionality}
	When the user clicks on a button or list on the side bar, it will pass a route to the scope variable in the view. Once that route is called, it will use the ``ng-include’’ function with the given route to load and inject the appropriate contents using Angular.
    \subsubsection{Rationale} 
	From a developer standpoint, using context views will dramatically reduce clutter both in the code and in the UI. There will only need to be one function call per container, verses having multiple static calls in the code.
