\documentclass[journal,10pt,onecolumn,compsoc]{IEEEtran} 
   ...
   \end{itemize}
    ...
    \begin{paddedtikzpicture}[node distance = 2cm]
        ...
        \path [line] (render) -- (edit); 
    \end{paddedtikzpicture}  
    \caption{A functional representation of creating a dashboard object from user created block.} \label{fig}
    \end{figure}
    ...
    \begin{paddedtikzpicture}[node distance = 2cm]
        ...
        \path (story.east)+(2,0) node (retrieve) [datablock] {Retrieve all of user's dashboards};
        \path [line] (story) -- (retrieve);
        \path (retrieve.east)+(2,0) node (add) [datablock, text width=8em] {Save selected dashboards into story};
	...
	\subsubsection{Element}
	A story will be within the navigation side bar. On the ``Stories'' tab itself, a user can click on all of the created stories.
	\subsubsection{Functionality}
    When a user selects a story from the list, it will retrieve all of the stored dashboards. Once the dashboards are retrieved, it will display the relevant data on the screen. If a user clicks a different story, it will remove the current set of data and load the new data.
	\subsubsection{Rationale}
